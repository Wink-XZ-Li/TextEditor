...
class LineNumberRulerView: NSRulerView {
   ...
   // var rulerViewWidth: CGFloat
    init(textView: NSTextView) {
        ...
        self.font = NSFont.systemFont(ofSize: 12)//textView.font ?? NSFont.systemFont(ofSize: NSFont.smallSystemFontSize)
        ...
        if let textView = self.clientView as? NSTextView {
            if let layoutManager = textView.layoutManager {
                ...
                let drawLineNumber = { (lineNumberString: String, yValue: CGFloat) -> Void in
                    ...
                    let xValue: CGFloat = 5//5//35 - attributeString.size().width
